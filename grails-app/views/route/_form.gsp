...
<div class="fieldcontain ${hasErrors(bean: routeInstance, field: 'region', 'error')} required">
	...
	<g:select id="region" name="region.id" from="${happytrails.Region.list()}" optionKey="id" required="" value="${routeInstance?.region?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: routeInstance, field: 'photo', 'error')} ">
	...
	<g:select id="photo" name="photo.id" from="${happytrails.Photo.list()}" optionKey="id" value="${routeInstance?.photo?.id}" class="many-to-one" noSelection="['null': '']"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: routeInstance, field: 'creationDate', 'error')} ">
	...
	<g:datePicker name="creationDate" precision="day"  value="${routeInstance?.creationDate}" default="none" noSelection="['': '']" />
</div>
...
<li class="add">
<g:link controller="comment" action="create" params="['route.id': routeInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'comment.label', default: 'Comment')])}</g:link>
</li>
</ul>
...
<div class="fieldcontain ${hasErrors(bean: routeInstance, field: 'directions', 'error')} ">
	...
	<g:select name="directions" from="${happytrails.Direction.list()}" multiple="multiple" optionKey="id" size="5" value="${routeInstance?.directions*.id}" class="many-to-many"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: routeInstance, field: 'ratings', 'error')} ">
	...
	<g:select name="ratings" from="${happytrails.Rating.list()}" multiple="multiple" optionKey="id" size="5" value="${routeInstance?.ratings*.id}" class="many-to-many"/>
</div>
