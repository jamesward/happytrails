...
<g:set var="breadcrumb" scope="request">
    <ul class="breadcrumb">
        <li><a class="home" href="${createLink(uri: '/')}"><g:message code="default.home.label"/></a> <span
    ...
    <sec:ifLoggedIn>
        <g:form style="position: absolute">
            <fieldset>
                ...
                <g:link action="edit" id="${regionInstance?.id}" class="btn btn-mini"><g:message
                        code="default.button.edit.label" default="Edit"/></g:link>
                <g:actionSubmit action="delete" class="btn btn-mini"
                                value="${message(code: 'default.button.delete.label', default: 'Delete')}"
                                onclick="return confirm('${message(code: 'default.button.delete.confirm.message', default: 'Are you sure?')}');"/>
                <g:link class="btn btn-mini" controller="route" action="create"
        ...
        <table class="routes table table-condensed" style="width: 600px">
            ...
            <g:each in="${regionInstance.routes}" status="i" var="route">
                <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                    <td><link:region region="${regionInstance.seoName}"
                                     route="${route.seoName}">${route.name}</link:region></td>
